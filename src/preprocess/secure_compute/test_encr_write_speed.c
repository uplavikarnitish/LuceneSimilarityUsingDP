...
int main(int argc, char *args[])
{
	...
	if ( argc < 2 )
	{
		fprintf(stderr, "EBADARG!! Usage:%s <key_file_name_loc>\n", args[0]);
		return -1;
	}
	strncpy(g_key_file_name, args[1], sizeof(g_key_file_name));
	init();
	fp = fopen("abc_del_this.txt", "w");
	mpz_t (big_num);
	mpz_t (big_num2);
	...
	mpz_set_ui(big_num2, 1323);
	encrypt(big_num, 234234);
	begin = clock();
	for ( i=0; i<iter; i++ )
	{
		//encrypt(big_num, 234234);
		//mpz_powm_ui(big_num, big_num, 500000, n_square);
		mpz_powm(big_num, big_num, big_num2, n_square);
		//gmp_fprintf(fp, "%Zd\n", big_num);

	}
	end = clock();
	printf("\nFor %d iterations, exp. time taken: %f (seconds)\n", iter, (((double)(end-begin))/CLOCKS_PER_SEC));
	mpz_clear(big_num);
	mpz_clear(big_num2);
	fflush(fp);
	sync();
